...
## Read in Principia and run basic cleaning
...
principia_dfm <- principia %>% corpus %>%
  ...
  tokens_remove(stopwords("en"), padding = TRUE) %>% #Padding = True leaves a placeholder where stopwords previously were, useful for making bigrams
...
```{r}
## Bridging function to convert to object we can use with stm package
...
## No covariates, so estimates a correlated topic model (allows for correlation between the topics)
...
```{r}
cloud(ctmodel, topic = 1, scale = c(2,.25)) ## Maths/grammar
cloud(ctmodel, topic = 2, scale = c(2,.25)) ## Astronomy
cloud(ctmodel, topic = 3, scale = c(2,.25)) ## Mechanics
```
...
beta <- exp(ctmodel$beta$logbeta[[1]]) ## package saves only log beta, so we have to manually exponentiate
dim(beta)
...
plot_data[,c("Maths", "Mechanics", "Astronomy")] <-
  rollmean(plot_data[,c("Maths", "Mechanics", "Astronomy")], 10, ## 10 page rolling average so not too erratic in plots
