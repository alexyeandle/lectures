...
stmodel <- stm(documents = stm_input$documents, vocab = stm_input$vocab,
                     K = 50, prevalence =~ party + s(day_of_year), # s() means "smoothed" version of this
               data = stm_input$meta, verbose = FALSE, init.type = "Spectral")

## Prevalence covariates: might be different topic shares in each document given covariates
## We don't use topic content covariates (so we assume the words that construct each topic are constant)
```
...
## i.e. THIS IS WHERE WE GET WHETHER STAT. SIG DIFFERENCES ACROSS GROUPS
